public class _34_find_first_and_last_position_of_target_in_sorted_array {
...
// If A[mid] < target, then the range must begins on the right of mid (hence i =
...
// range, eventually we will be at a situation where there are only two elements
// in the search range. Suppose our target is 5, then we have only 7 possible
// cases:
...
// If A[mid] < target, then the range must begins on the right of mid (hence i =
// mid+1 for the next iteration)
// If A[mid] = target, then the range must begins on the right of or at mid (i=
// mid)
...
// mid = (i+j)/2+1
// Why does this trick work? When we use mid = (i+j)/2, the mid is rounded to
// the lowest integer. In other words, mid is always biased towards the left.
// This means we could have i == mid when j - i == mid, but we NEVER have j ==
// mid. So in order to keep the search range moving, you must make sure the new
// i is set to something different than mid, otherwise we are at the risk that i
// gets stuck. But for the new j, it is okay if we set it to mid, since it was
// not equal to mid anyways. Our two rules in search of the left boundary happen
...
// get stuck when we set the new i to i = mid. The easiest way to achieve this
// is by making mid biased to the right, i.e. mid = (i+j)/2+1.
